...
const baseAnswers: Record<string, string> = {
  ...
  "Does `HashSet` allow `null` elements? Does `TreeSet`?":
    "`HashSet` allows **one `null`**, straight from the backing `HashMap` allowing one null key. `TreeSet` under natural ordering does **not** — it places elements by calling `compareTo`, and `compareTo(null)` throws `NullPointerException`. Same story as `HashMap` and `TreeMap`, for the same underlying reason.\n\nThe rule is really about the **comparator**, not the class. Give a `TreeSet` a null-tolerant one — `new TreeSet<>(Comparator.nullsFirst(Comparator.naturalOrder()))` — and it will happily take a null, because now there's a defined answer for where null sorts. Worth knowing so you understand what the restriction actually is, though putting nulls in a sorted set is a decision you'd want a good reason for.",
  ...
  "Give examples of each from the JDK.":
    "**Checked** — `IOException`, `SQLException`, `ClassNotFoundException`. The environment can cause these no matter how correct your code is, so the compiler makes you catch them or declare them. **Unchecked** — `NullPointerException`, `IllegalArgumentException`, `ArrayIndexOutOfBoundsException`, `ArithmeticException`. These extend `RuntimeException` and mean somebody wrote a bug; no amount of catching fixes the cause.\n\nThe split reads as \"the caller can recover\" versus \"the caller has a bug\".\n\nThere's a third branch the two-way split hides, though. **`Error`** — `OutOfMemoryError`, `StackOverflowError`, `NoClassDefFoundError` — is also unchecked, but it's neither a bug you fix at the throw site nor something you catch. The JVM is telling you it can't continue. That's why \"unchecked\" precisely means `RuntimeException` **and** `Error`, both branches of unchecked `Throwable` — and it's the same definition Spring's `@Transactional` uses when it decides what rolls back.",
