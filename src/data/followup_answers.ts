...
const baseAnswers: Record<string, string> = {
  ...
  "Why doesn't Java allow multiple class inheritance?":
    "Because classes carry **state and constructors**, and inheriting two of them is ambiguous. If `Employee` extended both `Person` and `Auditable` and each declared a `name` field with its own constructor, the JVM couldn't say which `name` an instance holds or what order the constructors run — that's the **diamond problem**, and C++ solves it with virtual inheritance rules most people get wrong. Interfaces are safe to multiply-inherit because they hold **no instance state**: a `default` method is behaviour only, so a collision has exactly one resolution and the compiler forces you to write it via `Interface.super.method()`. You get the mixin benefit without the ambiguity — which is why `implements A, B, C` is fine but `extends A, B` isn't.",
