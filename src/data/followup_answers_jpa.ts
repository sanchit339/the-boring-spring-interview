...
export const followupAnswersJpa: Record<string, string> = {
  ...
  "How would you detect N+1 in logs or with a tool?":
    `Enable **\`spring.jpa.show-sql=true\`** and **\`logging.level.org.hibernate.SQL=DEBUG\`** — if loading 20 orders generates 21 SQL statements in your console, that's N+1. For cleaner detection, add **\`spring.jpa.properties.hibernate.generate_statistics=true\`** and check \`Session Metrics\` in logs. In production, use **Datadog APM**, **p6spy** (a JDBC proxy that logs every query), or **Hibernate's slow query log** (\`hibernate.session.events.log.LOG_QUERIES_SLOWER_THAN_MS\`). The answer that separates a 2YoE dev from a senior one is **automating** it: assert the query count in an integration test so N+1 can never regress silently.
