...
export const followupAnswersJpa: Record<string, string> = {
  ...
  "Is Hibernate a JPA implementation or a separate API?":
    "Hibernate is **both** — it's a full ORM with its own proprietary API (`Session`, `Criteria`, `HQL`) that predates JPA, and it also **implements** the JPA standard (`EntityManager`, `JPQL`, `@Entity`). The JPA spec (`jakarta.persistence`, and `javax.persistence` before Boot 3) is an interface; Hibernate is the most popular implementation of that interface. When Spring Boot adds `spring-boot-starter-data-jpa`, it pulls in Hibernate as the JPA provider by default.",
  ...
  "Which methods does each interface add?":
    "`CrudRepository` gives you `save`, `findById`, `findAll`, `delete`, `deleteById`, `existsById`, and `count` — the absolute CRUD minimum. `PagingAndSortingRepository` adds `findAll(Pageable)` and `findAll(Sort)`. `JpaRepository` adds `saveAll`, `flush`, `saveAndFlush`, `deleteAllInBatch`, and `getReferenceById` — plus `findAll` returns `List` instead of `Iterable`, which is more practical. Two Boot 3 corrections worth knowing: **since Spring Data 3.0 `PagingAndSortingRepository` no longer extends `CrudRepository`** (they're independent now, with `ListCrudRepository`/`ListPagingAndSortingRepository` as the List-returning variants that `JpaRepository` actually extends). Separately, `getById`/`deleteInBatch` are **deprecated** in favour of `getReferenceById`/`deleteAllInBatch`.",
  ...
  "How do you run modifying queries (`@Modifying`) with `@Transactional`?":
    "Any `@Query` that is a `DELETE`, `UPDATE`, or `INSERT` must be annotated with **`@Modifying`** so Spring Data knows to use `executeUpdate()` instead of `getResultList()`. It also **must** be inside a `@Transactional` — without it you get a `TransactionRequiredException` at runtime. Put `@Transactional` on the calling **service** method rather than the repository — the service is where the unit of work belongs. Also add `@Modifying(clearAutomatically = true)` if you read the same entities afterward in the same transaction — otherwise Hibernate's first-level cache returns stale data that doesn't reflect the bulk update.",
  ...
  "How would you detect N+1 in logs or with a tool?":
    `Enable **\`spring.jpa.show-sql=true\`** and **\`logging.level.org.hibernate.SQL=DEBUG\`** — if loading 20 orders generates 21 SQL statements in your console, that's N+1. For cleaner detection, add **\`spring.jpa.properties.hibernate.generate_statistics=true\`** and check \`Session Metrics\` in logs. In production, use **Datadog APM**, **p6spy** (a JDBC proxy that logs every query), or **Hibernate's slow query log** (\`hibernate.session.events.log.LOG_QUERIES_SLOWER_THAN_MS\`). Better still, **automate** it: assert the query count in an integration test so N+1 can never regress silently.
...
The point: don't discover N+1 in prod — it's a cliff, not a slope. It looks fine on 20 dev rows and melts the DB on 20,000 prod rows.`,
  ...
  "How do `JOIN FETCH`, `@EntityGraph`, and batch fetching help?":
    "**`JOIN FETCH`** in JPQL pulls the association in a single query — `SELECT o FROM Order o JOIN FETCH o.items WHERE o.id = :id`. **`@EntityGraph`** does the same declaratively on a repository method without changing the query string — `@EntityGraph(attributePaths = {\"items\"})`. **Batch fetching** (`@BatchSize(size = 25)` on the collection or globally via `hibernate.default_batch_fetch_size`) groups lazy loads into batches — instead of N individual `SELECT`s you get `WHERE id IN (1,2,...,25)`. `JOIN FETCH` is the strongest (1 query), `@EntityGraph` is cleaner for reuse, and batch fetching is a cheap global fix that reduces N+1 without touching queries. **The trap to call out:** you can only `JOIN FETCH` **one** collection per query — fetching two `List` associations at once throws `MultipleBagFetchException`. Fix it by fetching one collection and letting `@BatchSize` handle the second, or by changing `List` to `Set`. Also remember `JOIN FETCH` + `Pageable` makes Hibernate paginate **in memory** (`HHH000104`), loading the whole result set — use `@EntityGraph` with a two-step ID query when you need both paging and fetching.",
  ...
  "What can go wrong with `REQUIRES_NEW` when the connection pool is small?":
    "You deadlock the pool. `REQUIRES_NEW` **suspends** the outer transaction but doesn't release its connection, then asks the pool for a second one — so every thread inside that call holds **two connections at once**. With HikariCP's default `maximum-pool-size=10`, ten concurrent requests reaching the `REQUIRES_NEW` call hold all ten connections as suspended outers, and all ten inner transactions queue for a connection that can never be freed. You get `Connection is not available, request timed out after 30000ms` under load and nothing at all in testing.\n\nThe fix is either sizing the pool for the real nesting depth (roughly `threads x connections-per-thread`) or not nesting at all — publish an event and let a `@TransactionalEventListener(phase = AFTER_COMMIT)` do the independent write on its own. Trade-off: if the DB connection drops mid-audit, the outer transaction might succeed and the audit fail — log to a separate store (Kafka, dedicated audit DB) for true reliability.",
  ...
  "How do you set isolation on `@Transactional`?":
    "Pass the `isolation` attribute: `@Transactional(isolation = Isolation.READ_COMMITTED)`. Spring maps it to the JDBC `Connection.setTransactionIsolation(...)` call. The catch: not all databases support all isolation levels — setting `SERIALIZABLE` on a DB or driver that doesn't support it either silently falls back or throws. Always verify your DB supports the level you set. The trap: isolation is a property of the **physical transaction**, so if an inner `REQUIRED` method declares `SERIALIZABLE` and joins an outer `READ_COMMITTED` transaction, the inner setting is **silently ignored**. Spring only throws `IllegalTransactionStateException` if you set `validateExistingTransaction=true` on the transaction manager, which is off by default. If you genuinely need a different level, you need `REQUIRES_NEW` to get a separate physical transaction and connection.",
